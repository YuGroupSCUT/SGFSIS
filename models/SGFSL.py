        ...
        if gen_proto:
            ...
            with torch.no_grad():
                ...
                for tb in base_list:
                    ...
                    base_vec[tb] = 1
                    raw_feat = gened_proto[tb].clone()
                    # # relation network
                    # all_feat = torch.cat([raw_feat, tmp_gened_proto[tb]], 0).unsqueeze(0)  # 1, 1024
                    # ratio = torch.sigmoid(self.gamma_conv(all_feat))[0]  # n, 512

                    # cosine similarity
                    raw_feat_norm = F.normalize(raw_feat, 2, -1)
                    tmp_gened_feat_norm = F.normalize(tmp_gened_proto[tb], 2, -1)
                    weight = (raw_feat_norm * tmp_gened_feat_norm).sum(-1)
                    ratio = weight * (weight > 0).float()
                    gened_proto = gened_proto * (1 - base_vec) + (
                            (raw_feat * ratio + tmp_gened_proto[tb] * (1 - ratio)).unsqueeze(0) * base_vec)
